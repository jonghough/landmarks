...
export class InfoBillboard {
  ...
  constructor(
    globalConfig: GlobalConfig,
    readonly infoDialogOpenCallback: (title: string, text: string, homePage: string) => void,
    centerLatitude: number,
    centerLongitude: number,
  ) {
  ...
  hide() {
    if (this.tileInfoPlane) this.tileInfoPlane.isVisible = false;
  }
  show() {
    ...
    if (this.tileInfoPlane) this.tileInfoPlane.isVisible = true;
  }
  dispose() {
    this.adt?.dispose();
